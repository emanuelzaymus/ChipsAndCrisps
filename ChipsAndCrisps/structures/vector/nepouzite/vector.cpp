...
namespace structures {
	...
	void Vector::copy(const Vector& src, const int srcStartIndex, Vector& dest, const int destStartIndex, const int length)
	{
		...
		if (src == dest && srcStartIndex < destStartIndex ? (srcStartIndex + length > destStartIndex) : (srcStartIndex - length < destStartIndex))
			memmove(&dest + destStartIndex, &src + srcStartIndex, length);
		else
			memcpy(&dest + destStartIndex, &src + srcStartIndex, length);	//todo ... dest.getBytePointer(destStartIndex) ???
	}
